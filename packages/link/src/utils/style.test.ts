...
describe('Test style utils', () => {
  ...
  test('verify function return nothing on missing query param', () => {
    const link = 'https://some.domain&other=12'
    const received = getLinkStyle(link)
    expect(received).toBeNull()
  })
  test('verify function return nothing on wrong encoded value', () => {
    const link = 'https://some.domain&link_style=eyJpciI6IDI0LCAiaW8iOiAwL'
    const received = getLinkStyle(link)
    expect(received).toBeUndefined()
  })
  test('verify function return nothing on empty link', () => {
    const link = ''
    const received = getLinkStyle(link)
    expect(received).toBeNull()
  })
  test('verify function return correct number', () => {
